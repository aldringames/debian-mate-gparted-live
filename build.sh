...
cd $HOME/live
mkdir -p $HOME/live/{prod/{EFI/boot,boot/grub/{fonts,x86_64-efi},isolinux,live},tmp}
echo "Copying override.conf to chroot/etc/systemd/system/console-getty.service.d/..."
mkdir -p chroot/etc/systemd/system/console-getty.service.d/
cp -f $WORK/override.conf chroot/etc/systemd/system/console-getty.service.d/
...
echo "Removing docker env on chroot"
rm chroot/.dockerenv
...
grub-mkfont -o prod/boot/grub/fonts/arial.pf2 -s 15 $WORK/arial.ttf &> /dev/null
